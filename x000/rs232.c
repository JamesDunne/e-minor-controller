...
extern rom struct{byte bLength;byte bDscType;word string[10];}DevChem;
...
//Polling command:
	if (!USBQuery && RxBuffer[1] == 0x03 || \
		...
		case 42:
			//iDeviceChemistry
			ResponseType = DATA_RETURNED;
			TxBuffer[2] = 5;			//number of bytes in response
			if (USBQuery) {
				TXENQ(INDEX_DEVCHEM);
				break;
			}			
			...
			StringInEEPROM = false;
			StringInRAM = false;
			tempreg.s_form = (unsigned short)(&DevChem)+2;
			StringPtr_H = tempreg.b_form.high;	//(backwards for some crazy reason)
			StringPtr = tempreg.b_form.low;	//Send out serial string.
			break;
