...
{    
    ...
    0x09AE,     // idVendor - 0x09AE is Tripplite's Vendor ID
    0x4429,		// idProduct
...
// Interface descriptor (Interface 0 = DemoKit)
	...
	0x01,		// bNumEndpoints: one endpoints used    ** Renaud : changed from 2 to 1
	0x03,		// bInterfaceClass: HID
	0x00,		// bInterfaceSubClass: ** Renaud : changed from 1 (Boot Interface Subclass in tripplite descriptor)to 0 : no boot subclass
...
//Device Chemistry String
rom struct{byte bLength;byte bDscType;word string[4];}DevChem={
sizeof(DevChem),DSC_STR,
'P','b','A','C'};

//Product String
struct{byte bLength;byte bDscType;word string[21];}PrdString={
sizeof(PrdString),DSC_STR,
'J','o','e',0x2019,'s',' ','S','c','o','p','e',' '};

//Serial Number
struct{byte bLength;byte bDscType;word string[16];}SerialNum={
...
/*
	...
	0x85,  RID_RECEIVE       // REPORT_ID (1)  - streaming UART rx data
	0x09,  0x01                // USAGE (Vendor Usage 1)
	0x15,  0x00                // LOGICAL_MINIMUM (0)
	0x26,  0xff, 0x00           // LOGICAL_MAXIMUM (255)
	0x75,  0x08                // REPORT_SIZE (8) = bits 
	0x95,  EP1_REPORT_LENGTH // REPORT_COUNT (7) = 1 report id byte + 7 data bytes
	0x81,  0x06                // INPUT (Data,Var,Rel)
	0x09,  0x01                // USAGE (Vendor Usage 1)   
	0xb1,  0x06                // FEATURE (Data,Var,Rel) 

	0x85,  RID_COMMAND       // REPORT_ID (3) Command (setreport)
	0x09,  0x01               // USAGE (Vendor Usage 1)
	0x15,  0x00               // LOGICAL_MINIMUM (0)
	0x26,  0x08, 0x00           // LOGICAL_MAXIMUM (255)
	0x75,  0x08               // REPORT_SIZE (8) bits
	0x95,  0x01               // REPORT_COUNT (1) byte
	0x09,  0x01               // USAGE (Vendor Usage 1)
	0xb1,  0x06               // FEATURE (Data,Var,Rel) 
        
	0x85,  RID_TRANSMIT      // REPORT_ID (2) serial transmit these bytes (SetReport)
	0x09,  0x03               // USAGE (Vendor Usage 1)
	0x96,  0x07, 0x00          // REPORT_COUNT (7) 1 count byte, 6 data
	0xb1,  0x06               // FEATURE (Data,Var,Rel)   
...
rom const unsigned char *rom USB_SD_Ptr[]={&sd000,&MfrString,&PrdString,&SerialNum,&DevChem};
